...
@section('content-header')
    <h1>
        {{ trans('tradevillage::lessons.title.create lessons') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.tradevillage.lessons.index') }}">{{ trans('tradevillage::lessons.title.lessons') }}</a></li>
        <li class="active">{{ trans('tradevillage::lessons.title.create lessons') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.tradevillage.lessons.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('tradevillage::admin.lessons.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
                    <div class="box-body">
                        <div class="form-group{{ $errors->has("refer_doc") ? " has-error" : "" }}">
                            ...
                            {!! Form::textarea("refer_doc", old("refer_doc"), ["class" => "form-control", "placeholder" => trans("tradevillage::lessons.form.refer_doc")]) !!}
                        ...
                        <div class="form-group{{ $errors->has("refer_video") ? " has-error" : "" }}">
                            ...
                            {!! Form::textarea("refer_video", old("refer_video"), ["class" => "form-control", "placeholder" => trans("tradevillage::lessons.form.refer_video")]) !!}
