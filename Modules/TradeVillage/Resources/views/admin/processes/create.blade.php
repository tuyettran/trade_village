...
@section('content-header')
    <h1>
        {{ trans('tradevillage::processes.title.create process') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.tradevillage.process.index') }}">{{ trans('tradevillage::processes.title.processes') }}</a></li>
        <li class="active">{{ trans('tradevillage::processes.title.create process') }}</li>
    </ol>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'b', route: "<?= route('admin.tradevillage.process.index') ?>" }
                ]
            });
        });
    </script>
