...
@section('content-header')
    <h1>
        {{ trans('tradevillage::videos.title.create video') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.tradevillage.video.index') }}">{{ trans('tradevillage::videos.title.videos') }}</a></li>
        <li class="active">{{ trans('tradevillage::videos.title.create video') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.tradevillage.video.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('tradevillage::admin.videos.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
                    <div class="box-body">
                        <div class="form-group{{ $errors->has("link") ? " has-error" : "" }}">
                            ...
                            {!! Form::text("link", old("link"), ["class" => "form-control", "placeholder" => trans("tradevillage::videos.form.link")]) !!}
                        ...
                        <div class="form-group{{ $errors->has("chapter") ? " has-error" : "" }}">
                            ...
                            {!! Form::text("chapter", old("chapter"), ["class" => "form-control", "placeholder" => trans("tradevillage::videos.form.chapter")]) !!}
