			...
			return this.each(function() {
				...
				var openAZ = function(e, inp) {
					...
					if (inp.ajaxZoomOpenMode == 'iframe') {
						...
						_wrapIframe = $('<iframe src="'+op.iframeLink+iParam+'" frameborder="0" class="azHoverThumbWrap" allowfullscreen>')
						.appendTo(_parent);
				...
				var recalculate = function() {
					...
					if (op.parentHeightRatio) {
						if (op.parentHeightRatio == 'auto') {
							var hh = wrapRef.innerWidth() * realH/realW;
							if (hh > realH) {
								hh = realH;
							}
						} else {
							var hh = wrapRef.innerWidth() * op.parentHeightRatio;
						}
					...
					} else if (op.parentWidthRatio) {
						if (op.parentWidthRatio == 'auto') {
							var ww = _parent.innerHeight() * realW/realH;
						} else {
							var ww = _parent.innerHeight() * op.parentWidthRatio;
						}
